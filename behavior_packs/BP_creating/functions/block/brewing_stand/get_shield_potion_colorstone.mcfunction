...
# 如果缺少彩岩，则提示玩家不能酿造
execute unless entity @s[hasitem={item=aw:build3}] run tellraw @s {"rawtext":[{"text":"§e你缺少彩岩，不能酿造护盾药水！"}]}
execute unless entity @s[hasitem={item=aw:build3}] run scoreboard players set canBrewPotion data 0

# 否则获得药水并清除空瓶子
execute if score canBrewPotion data matches 1 run clear @s glass_bottle 0 1
execute if score canBrewPotion data matches 1 run clear @s aw:build3 0 1
execute if score canBrewPotion data matches 1 run give @s aw:potion_shield
