...
execute if score 7_fortress_book data matches 0 if block -286 -49 60 chest if blocks -286 -49 60 -286 -49 60 -286 -51 60 all run function lib/utils/music/queue
...
execute if score 7_fortress_key data matches 0 if block -270 -49 74 chest if blocks -270 -49 74 -270 -49 74 -234 -9 179 all run function lib/utils/music/queue
...
#把田萍和王予凡救出来
execute if score 7_fortress_help data matches 0 if entity @a[x=-268,y=-49,z=59,dx=2,dy=3,dz=2] run function lib/modify_data/states/timeline/enable_time_lapse
...
#从BOSS战离开（第一次离开时）
execute if score 7_fortress_boss data matches 1 if entity @a[x=-225.5,y=-49,z=82.5,r=1] run function lib/modify_data/states/timeline/enable_time_lapse
