...
execute unless entity @s[hasitem={item=pufferfish}] run scoreboard players set could_brew_potion data 0

# 如果缺少玻璃瓶，则提示玩家不能酿造
execute unless entity @s[hasitem={item=glass_bottle}] run tellraw @s {"rawtext":[{"text":"§e你缺少玻璃瓶，不能酿造恢复药水！"}]}
execute unless entity @s[hasitem={item=glass_bottle}] run scoreboard players set could_brew_potion data 0

# 如果材料充足，获得药水，清除河豚和空瓶子
execute if score could_brew_potion data matches 1 run clear @s pufferfish 0 1
execute if score could_brew_potion data matches 1 run clear @s glass_bottle 0 1
execute if score could_brew_potion data matches 1 run give @s aw:potion_regeneration
