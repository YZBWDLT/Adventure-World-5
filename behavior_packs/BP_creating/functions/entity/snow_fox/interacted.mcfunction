...
execute if entity @s[x=-308,y=8,z=53,r=1] if score schellonBrush itemState matches 0 run tellraw @a {"rawtext":[{"text":"§e雪狐狐送给你一件礼物！"}]}
execute if entity @s[x=-308,y=8,z=53,r=1] if score schellonBrush itemState matches 1 run tellraw @a {"rawtext":[{"text":"§a[我]这支笔上写着，要配合染料使用。"}]}
execute if entity @s[x=-308,y=8,z=53,r=1] if score schellonBrush itemState matches 0 run scoreboard players set schellonBrush itemState 1
